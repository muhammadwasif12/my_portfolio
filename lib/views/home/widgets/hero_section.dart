...
class _HeroSectionState extends State<HeroSection> with TickerProviderStateMixin {
  // ... Saare animation controllers aur initState bilkul waise hi rahenge ...
  // ... (Aapka purana initState code yahan paste karein) ...
  ...
  void initState() {
    ...
    _floatingController = AnimationController(duration: const Duration(milliseconds: 8000), vsync: this);
    _pulseController = AnimationController(duration: const Duration(milliseconds: 2000), vsync: this);
    _rotationController = AnimationController(duration: const Duration(milliseconds: 20000), vsync: this);
    _profileController = AnimationController(duration: const Duration(milliseconds: 3000), vsync: this);
    _fadeAnimation = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(parent: _controller, curve: const Interval(0.0, 0.6, curve: Curves.easeOut)));
    _slideAnimation = Tween<Offset>(begin: const Offset(0, 0.5), end: Offset.zero).animate(CurvedAnimation(parent: _controller, curve: const Interval(0.2, 0.8, curve: Curves.elasticOut)));
    _scaleAnimation = Tween<double>(begin: 0.8, end: 1.0).animate(CurvedAnimation(parent: _controller, curve: const Interval(0.4, 1.0, curve: Curves.elasticOut)));
    ...
    _profileAnimation = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(parent: _profileController, curve: const Interval(0.0, 1.0, curve: Curves.elasticOut)));
    _controller.forward();
  ...
  // ... Baaki saare _build methods (jaise _buildAnimatedProfile, etc.) waise hi rahenge ...
  // Lekin _buildEnhancedBackground, _buildFloatingBubble, aur _buildRotatingRing ko update karna hoga
  // ... (Aapke purane _build methods yahan paste karein, lekin neeche diye gaye 3 methods ko naye se replace karein) ...
  ...
  // Yahan par baaki saare _build methods paste karein (_buildAnimatedProfile, _buildAnimatedName, etc.)
  Widget _buildAnimatedProfile(bool isMobile, bool isTablet) {
    ...
    return AnimatedBuilder(
      animation: Listenable.merge([_profileAnimation, _pulseAnimation, _rotationAnimation]),
      builder: (context, child) {
        return Transform.scale(
          ...
          child: Stack(
            ...
            children: [
              Transform.rotate(
                ...
                child: Container(width: outerRingSize, height: outerRingSize, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(width: isMobile ? 2 : 3, color: AppTheme.primaryBlue.withOpacity(0.4)))),
              ),
              Container(
      ...
      builder: (context, child) {
        return Container(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Expanded(child: Transform.scale(scale: _scaleAnimation.value, child: _buildGitHubButton(isMobile, isTablet))),
              SizedBox(width: isMobile ? 8 : 12),
              Expanded(child: Transform.scale(scale: _scaleAnimation.value, child: _buildLinkedInButton(isMobile, isTablet))),
            ],
          ),
        );
      },
